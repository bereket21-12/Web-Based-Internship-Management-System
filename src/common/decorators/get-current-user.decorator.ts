...
export const GetCurrentUser = createParamDecorator( // createParamDecorator is a function that takes a function as an argument and returns a new function.
    (data: keyof jwtPayloadWithRt | undefined, context: ExecutionContext) => { // The function that createParamDecorator takes as an argument takes two parameters: data and context. The data parameter is of type string or undefined, it is used to specify the property of the user object to return. The context parameter is of type ExecutionContext, it is used to get the request object.
        ...
        return request.user[data] // If the data parameter is specified, we return the property of the user object specified by the data parameter.
    }
)